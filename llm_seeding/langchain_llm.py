...
def _build_prompt(parser: PydanticOutputParser) -> ChatPromptTemplate:
    prompt = ChatPromptTemplate.from_messages(
        [
            ...
            (
                "human",
                "Existing heuristic candidates grouped by trade-off focus:\n{heuristics}\n\n"
                "Terminal node descriptions (please reference them when useful):\n"
                "{terminal_context}\n\n"
                "Task: For each heuristic group ({group_list}), propose exactly"
                " {suggestions_per_group} new heuristic expressions (total of"
                " {total_suggestions}). Each suggestion must identify its group and provide two"
