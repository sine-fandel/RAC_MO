...
if __name__ == "__main__":
    ...
    json_file = f"./results/training/nsw_Bitbrains_3OS_24000_affinity_communication_mincut_dual_run_{run}_core_{config['cpu_num']}.json"
    ...
    individual_results = toolbox.map(toolbox.evaluate_individual, pop)
    energy_list = []
    communication_list = []
    for ir in individual_results:
        energy_list.append(ir[0])
        communication_list.append(ir[1])
    energy_list = np.array(energy_list)
    communication_list = np.array(communication_list)

    # calculate the normalized sum up weight fitness
    for i in range(len(energy_list)):
        pop[i].fitness.values = eval(i, energy_list, communication_list)
    # ===========================================
    # ===========================================
    ...
    new_data = {"0": {"fitnees": logbook.chapters["fitness"].select("min")[-1], "diversity": len(set(fitness_list)), "time": logbook.chapters["fitness"].select("time")[-1], "best": str(best_individual(pop))}}
    ...
    for gen in range(1, generation_num):
        ...
        offspring = toolbox.select(pop[elitism_size : ], len(pop) - elitism_size)
        ...
        for i in range(len(pop)):
            fitness_list.append(pop[i].fitness.values[0])
        # update json file to save the best training result of each generation
        new_data = {str(gen): {"fitnees": logbook.chapters["fitness"].select("min")[-1], "time": logbook.chapters["fitness"].select("time")[-1], "best": str(best_individual(pop))}}
        with open(json_file, "r") as gen_file:
            data = json.load(gen_file)
            data["generation"].update(new_data)
        with open(json_file, "w") as gen_file:
