...
def parse_run_from_filename(fname: str):
    # expects pattern like nsw_Bitbrains_3OS_MOEA-D_{run}_core_{cpu}.json
    m = re.search(r"MOEA-D_(\d+)_core_(\d+)\.json$", fname)
    if m:
...
def get_output_file(cpu_num: int, run: int = None, gen: int = None):
    ...
    base_dir = os.path.join(output_dir, "output", "testing")
    ...
    if run is not None:
        ...
        out_file = os.path.join(
            out_dir,
            f"nsw_Bitbrains_{OS_DATASET}_MOEA-D_core_{cpu_num}_run_{run}{gen_suffix}.json",
        )
        ...
        out_file = os.path.join(
            out_dir,
            f"aggregate_nsw_Bitbrains_{OS_DATASET}_MOEA-D_core_{cpu_num}{gen_suffix}.json",
        )
        if not os.path.exists(out_file):
            with open(out_file, "w") as f:
                json.dump({"runs": {}}, f)
...
def build_test_mogp_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate trained MOEA-D individuals on test data"
    )
    parser.add_argument(
...
def resolve_train_dir(arg_value: str | None) -> str:
    ...
    base_train_root = os.path.join(base_dir, "output", "training")
