...
def log_all_fronts(
    ...
    if out_file is None:
        ...
        out_file = os.path.join(
            OUTPUT_DIR,
            f"nsw_Bitbrains_{OS_DATASET}_MOEA-D_core_{cpu_num}_run_{run}_gen_{gen}_fronts.json",
        )
...
def register_sim_evaluator(toolbox, gen: int):
    ...
    sim_state, input_containers, applications, applications_reverse, input_os = (
        training_simulation(case=gen, os_dataset=OS_DATASET)
    )
    toolbox.register(
        "evaluate_individual",
        eval_individual,
...
if __name__ == "__main__":
    ...
    json_file = os.path.join(
        OUTPUT_DIR,
        f"nsw_Bitbrains_{OS_DATASET}_MOEA-D_{run}_core_{config['cpu_num']}.json",
    )
