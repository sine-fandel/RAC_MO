...
from llm_seeding import generate_llm_seed_candidates
...
def log_all_fronts(
    ...
    if out_file is None:
        ...
        out_file = os.path.join(
            OUTPUT_DIR,
            f"nsw_Bitbrains_3OS_NSGP2LLM_core_{cpu_num}_run_{run}_gen_{gen}_fronts.json",
        )
...
def build_solve_mogp_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train NSGP2LLM and optionally dump all Pareto fronts for a generation",
    )
...
if __name__ == "__main__":
    ...
    json_file = os.path.join(
        OUTPUT_DIR,
        f"nsw_Bitbrains_3OS_NSGP2LLM_{run}_core_{config['cpu_num']}.json",
    )
    # Determine and record training case usage in the output JSON
    training_case_used = (
        str(args.training_case) if args.training_case is not None else "Dynamic"
    )
    ...
    for gen in range(loop_start, loop_end):
        ...
        # use LLM to get seeding individuals
        try:
            llm_candidates = generate_llm_seed_candidates(
                all_pop,
                pset,
        ...
        if llm_candidates:
            new_individuals = [
                creator.Individual(candidate) for candidate in llm_candidates
            ]
            eval_pop(new_individuals, toolbox)
            all_pop.extend(new_individuals)
